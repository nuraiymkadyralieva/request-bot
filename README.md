...
| `/manager_panel` | Просмотр всех заявок                   |
...
Для руководителя в проекте добавлена отдельная команда `/manager_panel`.

Менеджерская панель позволяет:
- просматривать нерассмотренные заявки
- фильтровать нерассмотренные заявки по срочности
- просматривать уже рассмотренные заявки
- фильтровать рассмотренные заявки по статусу
- открывать карточку конкретной заявки

Навигация внутри панели работает через inline-кнопки и редактирование одного сообщения, поэтому чат не засоряется служебными сообщениями при переходах между разделами и фильтрами.
